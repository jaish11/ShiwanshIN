CREATE PROCEDURE sp_ApplyJob  
(  
    @UserId INT,              
    @JobId INT,  
    @JobTitle NVARCHAR(200),  
    @JobType NVARCHAR(100),  

    @FullName NVARCHAR(200),  
    @Email NVARCHAR(200),  
    @Phone NVARCHAR(50),  

    @ResumeUrl NVARCHAR(MAX),  
    @ResumeFile NVARCHAR(MAX),  

    @University NVARCHAR(250),  
    @Degree NVARCHAR(200),  
    @Department NVARCHAR(200),  
    @GPA NVARCHAR(50),  
    @GraduationYear INT,  

    @CompanyName NVARCHAR(200),  
    @Position NVARCHAR(200),  
    @TotalExperience NVARCHAR(100),  
    @NoticePeriod NVARCHAR(50),  
    @ExperienceFrom DATETIME,  
    @ExperienceTo DATETIME,  

    @LinkedIn NVARCHAR(500),  
    @GitHub NVARCHAR(500)  
)  
AS  
BEGIN  
    INSERT INTO ApplyJobs  
    (  
        UserId,                
        JobId, JobTitle, JobType,  
        FullName, Email, Phone,  
        ResumeUrl, ResumeFile,  
        University, Degree, Department, GPA, GraduationYear,  
        CompanyName, Position, TotalExperience, NoticePeriod,  
        ExperienceFrom, ExperienceTo, LinkedIn, GitHub,  
        AppliedDate  
    )  
    VALUES  
    (  
        @UserId,             
        @JobId, @JobTitle, @JobType,  
        @FullName, @Email, @Phone,  
        @ResumeUrl, @ResumeFile,  
        @University, @Degree, @Department, @GPA, @GraduationYear,  
        @CompanyName, @Position, @TotalExperience, @NoticePeriod,  
        @ExperienceFrom, @ExperienceTo, @LinkedIn, @GitHub,  
        DATEADD(MINUTE, 330, GETUTCDATE())  
    );  
END;
